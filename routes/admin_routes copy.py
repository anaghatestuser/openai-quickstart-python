...
def init_app(app):
    ...
    @app.route('/approve_user/<username>', methods=['POST'])  # Using POST for actions that change state
    @login_required
    def approve_user(username):
        if not current_user.is_admin:
            abort(403)  # Only allow admins to approve users
        
        user_to_approve = User.query.filter_by(username=username).first()
        
        if not user_to_approve:
            flash('User not found.', 'error')
            return redirect(url_for('get_users'))  # Redirecting back to the admin dashboard
        
        user_to_approve.is_approved = True
        db.session.commit()

        # Send approval email
        msg = Message('Account Approved - K.I.S.S. IELTS', 
                    sender='ulysses@kissielts.com',
                    recipients=[user_to_approve.email])
        msg.body = ("Congratulations! Your K.I.S.S. IELTS account has been approved. "
                    "You can now access our platform at https://ielts-bxsh.onrender.com/. "
